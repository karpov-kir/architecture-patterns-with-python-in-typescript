...
export class TypeOrmProductsRepository implements ProductRepository {
  ...
  public async getByBatchReference(batchReference: string): Promise<Product> {
    const result = await this.session.manager
      .createQueryBuilder(BatchRecord, 'batch')
      .leftJoinAndSelect('batch.product', 'product')
      .select([])
      .addSelect('product.sku', 'sku')
      .where('batch.reference = :batchReference', { batchReference })
      .getRawOne<{ sku: string }>();

    if (!result) {
      throw new Error(`Could not get a product using ${batchReference} batch reference`);
    }
