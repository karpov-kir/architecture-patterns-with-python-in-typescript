...
import { BatchRecord, fromPersistance } from '../db/TypeOrmProductsRepository';
...
describe(TypeOrmUnitOfWork.name, () => {
  it('should save and retrieve a product', async () => {
    const unitOfWork = await TypeOrmUnitOfWork.create(messageBus);
  ...
  it(
    ...
    async () => {
      ...
      const unitOfWork1 = await TypeOrmUnitOfWork.create(messageBus);
      const unitOfWork2 = await TypeOrmUnitOfWork.create(messageBus);
      ...
      const finalUnitOfWork = await TypeOrmUnitOfWork.create(messageBus);
